...
[`08-studio-wireframes.md`](08-studio-wireframes.md).
...
## Library decision — dnd-kit, not pragmatic-drag-and-drop <!-- superseded-ok -->

**Revised.** An earlier draft chose the Atlassian adapter. <!-- superseded-ok --> Verification
...
**dnd-kit already solves this, and the earlier "experimental and undocumented" report was
wrong.** `@dnd-kit/dom`'s `getDocuments()` recursively walks `iframe`/`frame` elements,
catching the `SecurityError` on cross-origin access to skip them, and `PointerSensor` binds
`pointermove` / `pointerup` / `pointercancel` on every discovered same-origin document. It
is shipped and changelogged (PR #1517, "Support dragging across same-origin iframes"),
documented on dndkit.com, and covered by a Playwright regression test with a working
example. MIT.

Because it is pointer-based and same-origin, full drag state is retained throughout — no
MIME marshalling, no hover blind spot, no `dropEffect` sniffing.
...
| **dnd-kit** (MIT) | New `@dnd-kit/dom` has experimental same-origin frame traversal, undocumented and unreleased as a supported feature. Worth monitoring as a fallback. |
...
Pragmatic's approach is the one that fits: each document manages its own drag-and-drop, and
the **native HTML5 drag session** carries the drag across the boundary — not postMessage
relaying pointer coordinates. Atlassian ship a demonstrated example of this pattern. It is
also framework-agnostic with no React peer dependency, which suits a library that must not
constrain its consumer's React version.
...
flowchart LR
    subgraph parent["Studio shell (parent document)"]
        P["Block palette<br/>draggable()"]
        S["Selection + inspector"]
    end
    subgraph frame["iframe — consumer's real app, draft mode"]
        B["Halyard bridge script"]
        T["[data-halyard-slot]<br/>dropTargetForExternal()"]
    end

    P -->|"native HTML5 drag session"| T
...
- **Same-origin is likely required.** Pragmatic's cross-document example is confirmed
  same-origin only, and cross-origin is unsolved in every library reviewed. See open
  question 5.
...
- **Accessibility across a document boundary is untested territory.** Pragmatic ships a
  live-region package and recommends an action-menu reorder path alongside pointer drag —
  worth building that path from the start rather than retrofitting, since a keyboard drag
  that crosses into an iframe may simply not be announceable.
