...
summary: Panel layout, inspector controls, and key-state specs for the editing screen
status: draft
---

# Studio wireframes

Interface specification for the editing screen. Builds on the canvas architecture settled
in [`04-studio-ui.md`](04-studio-ui.md) — iframe-as-real-app, commit-on-blur, dnd-kit,
...
[`02-api-sketch.md`](02-api-sketch.md#control-resolution-ranked-testers-not-a-keyed-map).
...
## 3. Inspector controls, by measured field-type frequency

| Type | Share | Control | Appearance |
|---|---|---|---|
| `text` | 43% | `TextControl` | Single-line input, label above, char counter if `maxLength` set |
| `object` | 14% | `FieldsetControl` | Indented nested group, header = field label, recurses per sub-field |
| `list` | 12% | `RepeaterControl` | See [§3.1](#31-repeater-list) |
| `url` | 9% | `LinkControl` | `TextControl` + link icon, format-checked, `Open ↗` when valid |
| `html` | 8% | `RichTextControl` | Bordered box, fixed toolbar (bold/italic/link/list/heading), sanitized on commit |
| `uiBlocks` (slot) | 5% | `SlotSummaryControl` | Read-only summary card, edited via canvas — see below |
| `select` | 2% | `SelectControl` | Radio group ≤3 options, dropdown otherwise |
| `file` | 2% | `LinkControl` (default) | No built-in asset picker ships — see note |
| `boolean` | 2% | `ToggleControl` | Switch, label left, on/off |
| everything else | 2% | — | Degrades to read-only JSON, Storybook-style, rather than dropping the field |
...
consumer-registered `ImagePickerControl` via `hintIs("control", "image")`. Given `file` is
only 2% of fields, this is deliberate, not an oversight.
...
model itself is an open question upstream (domain model open question 6), so this spec does
not invent an expiry or force-release affordance.
...
(`"Bullets: maximum of 4 reached"`), and publish outcomes — per Pragmatic's live-region
recommendation cited in `04-studio-ui.md`.
