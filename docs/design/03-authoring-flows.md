...
summary: What an author does step by step, and where each flow is still unresolved
status: draft
---

# Authoring flows

What an author does, step by step, against the model in [`01`](01-domain-model.md), the API
in [`02`](02-api-sketch.md), and the canvas in [`04`](04-studio-ui.md). Every flow below is
Trigger → Steps → System → Failure modes. The failure modes are the point — several of them
are open questions or review findings rather than settled behavior, and are cited rather than
resolved here.
...
3. Optionally start from a preset instead of blank. "Preset" is this document's working
   name; the stored enum value is still `kind: "template"` pending the rename in
   [`05`](05-open-questions.md#6-template-collides-with-atomic-design).
...
- Blank start: `Document` created with `head: 1` and no route pointer, which is what "not
  ...
  ([`01`](01-domain-model.md#documentversion)).
- From a preset: the preset's tree is cloned and **every node id in the subtree is
  ...
  ([`01`](01-domain-model.md#node--flat-while-authoring-nested-once-published)).
...
| Duplicate / paste | Clone the subtree, remap **every** id via the shared clone utility — id is generated once and never regenerated for the original ([`01`](01-domain-model.md)). |
...
declaration greys out an invalid drop target during drag ([`02`](02-api-sketch.md#slots-articulate-structure)).
...
| Cross-document composition (copy a node from one open page into another) | Undesigned. The canvas is one iframe over one document at a time ([`04`](04-studio-ui.md)). |

## 3. Edit props

**Trigger:** select a node; the inspector renders its fields from `ui.order` / `ui.groups` /
`ui.fields`, control chosen by ranked tester ([`02`](02-api-sketch.md#control-resolution-ranked-testers-not-a-keyed-map)).
...
| Mode | Consequence |
|---|---|
| Invalid value against the block's schema | Validated on commit against the field's own sub-schema, and on blur against the whole node — the three tiers in [`02`](02-api-sketch.md#validation-happens-at-three-tiers-not-one). A draft may still hold invalid values indefinitely: blocking a save mid-edit is hostile, so publish is the gate, not save. |
...
1. Draft preview is a real server render — the studio's preview route is the same code
   path as the public catch-all, given a draft document ([`04`](04-studio-ui.md)).
...
reads an already-validated artifact ([`04`](04-studio-ui.md)).
...
([`01`](01-domain-model.md#compile-and-publish)). Unpublish is `store.unpublish(route)` —
...
adapters, and `store.publish()` is synchronous. Recorded as
[question 7](05-open-questions.md#7-scheduled-publishing), where the artifact model makes it
unusually safe if added — the artifact is compiled and validated before the schedule is set,
so firing cannot fail on a surprise validation error.
...
| Artifact pruning | Rollback depends on the target artifact still existing. Retention must respect a stated rollback window, and `publish()` must reject a missing hash rather than wiring a dead pointer — see [`adapters.md`](../../.claude/rules/adapters.md). No policy is set yet. |
...
([`01`](01-domain-model.md#node--flat-while-authoring-nested-once-published) — `ArtifactNode`
...
named layout slot ([`01`](01-domain-model.md#open-questions), question 2).
...
([`01`](01-domain-model.md#open-questions), question 6; [`05`](05-open-questions.md#13-model-level-questions)).
