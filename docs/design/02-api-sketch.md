...
Pseudocode, not final signatures. The point is the shape and where each concern lives.
...
[schema rule](../../.claude/rules/block-schemas.md) *requires* extracting shared
...
Block-level `data: "static" | "request"` was all-or-nothing <!-- superseded-ok --> — a hero with a static headline
...
resolved by the block at render. See [`Artifact`](01-domain-model.md#artifact) in
`01-domain-model.md`.
...
Adding the 123rd block costs pages that do not use it nothing. Contrast a registry spread
into every render call, where every page carries every component whether or not it appears —
which is why adding one icon set can degrade a whole site and force `next/dynamic` wrappers
onto every component defensively.
...
A design system may hold a hundred components while only a fraction are author-placeable —
a `MenuSeparator` is not a block. A block is an organism, and lives at the application's
