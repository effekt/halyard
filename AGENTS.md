...
- `pnpm catalog` — regenerate every `CATALOG.md`; pre-commit already does it for you
...
A list without a gate goes stale. `docs/gates.md` is hand-written and stays true because
`check-gate-table.mjs` reconciles it against what `pnpm verify` runs, so the answer to a list
is to gate it or generate it, not to have none.

| Asking | Read |
|---|---|
| What a package exports, and what each unit is for | The `CATALOG.md` beside it — [`packages/core/CATALOG.md`](packages/core/CATALOG.md) and its siblings |
| Which rules, agents and skills exist, and when each applies | [`.claude/CATALOG.md`](.claude/CATALOG.md) |

Both are generated by `scripts/catalog.mjs` from the declarations and frontmatter they
describe, regenerated in pre-commit, and asserted against a fresh generation by `pnpm test`.
Read one before writing a helper: the thing you are about to add is often already there.

One thing still has no mechanism. **A subagent's report** — it ends in a `## Findings`
