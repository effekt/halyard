...
How authors adjust spacing without breaking the design system. How an author adjusts
spacing and alignment without being able to break the design system.
...
closed union." That is false of real design systems, and was never the point. Any design
system with a healthy escape hatch exposes `className` as a genuine customization surface on
most of its components, and several require it outright — a grid needing a
...
props, which are free to be as open as they need to be.

## No wrapper element

Layout props merge onto the block's own root via **`className`**. The earlier draft said
"`className`/`style`/`ref`" — in practice `style` and `ref`/`forwardRef` are rare on
well-built design-system components, because they are the two escape hatches a component
author has the least reason to expose deliberately: `style` bypasses the token system
entirely, and `forwardRef` commits a component to exposing its underlying DOM node as part
of its API. `className` is the one that is reliably real, and the studio's overlay design
...
The evidence for avoiding a wrapper: components that accept an imposed CMS wrapper element
routinely need an opt-out, because the wrapper breaks assumptions the component was written
against. Both Builder.io and Puck shipped a mandatory wrapper first and had to retrofit an
opt-out (`noWrap`, and `inline`/`dragRef` respectively) once real usage ran into it.
...
case: any design system built around a `ReactNode` slot for images and rich body content
carries this risk on every block that uses one, which in practice is most content-heavy
blocks.
